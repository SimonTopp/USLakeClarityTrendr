...
## As a test, make centerpoints/disks
...
ggplot() + 
  geom_sf(data = region %>% st_simplify(dTolerance = 500), aes(fill = region)) +
  ...
  ggtitle('Random 18,000 Lake Sample and Ecoregions')

ggsave('figures/sampleNLA2012.png', width = 5, height = 6, units = 'in')
...
write_feather(lakes.out, 'out/Over10sqkm_cntr.feather')  
...
```{r}
files <- drive_ls('aquaModel_lakesNLA2007_cntr')
files <- files %>% filter(!name %in% list.files('lake_data/NLA2007_cntr'))
