...
LBEcoMeans <- LBPreds %>% st_set_geometry(NULL) %>%
  group_by(US_L3NAME) %>%
  summarise_at(vars(X1975MEAN:X2008MEAN), mean) %>%
...
check <- predicted
predicted <- predicted %>% bind_rows(predicted2)
...
p1 <- ggplot(lms %>% filter(source %in% c('RS', 'manual')), aes(x = Ecoregion, y = estimate, color = source)) + 
  geom_point() +
  geom_errorbar(aes(ymin = estimate - 2*std.error, ymax = estimate + 2*std.error)) +
  theme_bw() +
  theme(axis.text.x = element_blank(),
        axis.title = element_blank(),)+#element_text(angle = 30, hjust = 1)) + 
  scale_color_discrete('Source', labels = c('LB_Website', 'Our Preds')) +
  labs(title = 'Website')
