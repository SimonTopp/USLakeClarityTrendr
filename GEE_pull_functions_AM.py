...
def lakePull(image):
  ...
  clouds = f.eq(2).Or(f.eq(3)).Or(f.eq(4))
  ...
  if mask == "Pekel":
    pixOut = masking(image).addBands(f).addBands(d).addBands(h).addBands(hs)
  elif mask == 'dswe':
    pixOut = image.addBands(f).addBands(d).addBands(h).addBands(hs)\
    .updateMask(d.eq(1)).updateMask(clouds.eq(0)).set({'cScore':cScore})
      #.updateMask(f.lt(2))
  elif mask == 'none':
    pixOut = image.addBands(f).addBands(d).addBands(h).addBands(hs).set({'cScore':cScore})
  elif mask == 'land':
    pixOut = image.addBands(f).addBands(d).addBands(h).addBands(hs)\
    .updateMask(d.eq(0).Or(d.eq(4))).updateMask(clouds.eq(0)).set({'cScore':cScore})
  else:
    print('Enter valid pixel mask')
    ...
    #   h = CalcHillShades(image, lake.geometry()).select('hillShade')
    #   hs = CalcHillShadows(image, lake.geometry()).select('hillShadow')
    #   return image.addBands(f).addBands(d).addBands(h).addBands(hs).updateMask(d.eq(1)).updateMask(clouds.eq(0)).set({'cScore':cScore})
    # 
    # waterOut = waterOnly(image)

  #Collect median reflectance and occurance values
  lsout = pixOut.reduceRegion(ee.Reducer.median(), lake.geometry(), 30)
  
  #Collect reflectance and occurence st.dev.
  lsdev = pixOut.reduceRegion(ee.Reducer.stdDev(), lake.geometry(), 30)
  ...
  #set({'hylak_id':lake.get('hylak_id')})\
  output = shell.set({"blue": lsout.get('Blue')})\
  .set({'COMID':lake.get('COMID')})\
  .set({'sat': mission})\
  ...
  .set({"pixelCount": pixOut.reduceRegion(ee.Reducer.count(), lake.geometry(), 30).get('Blue')})\
  .set({'path': image.get('WRS_PATH')})\
  .set({'row': image.get('WRS_ROW')})\
  .set({'cScore': pixOut.get('cScore')})\
