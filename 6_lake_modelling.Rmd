...
boot.med <- function(data, indices){
  ...
  meds <- dt %>% group_by(dummy) %>% 
    summarise_at(vars(`1984`:`2019`), mean, na.rm = T) %>%
    select(-dummy)
...
```{r}
#Quickly look at the timeseries for a few individual lakes
sample <- summ.meds %>%
  group_by(COMID) %>%
  summarise(count = n()) %>%
  filter(count > 30) %>%
...
check<- summ.meds %>% filter(COMID %in% counts$COMID) %>% 
  ggplot(., aes(x = year, y = secchi.med)) + geom_line(aes(color = COMID, group = COMID)) + geom_smooth(se = F) + facet_wrap(~region, scales = 'free_y')

##Write a function to pull out man-kendall trends and slopes to connect with the wide data
...
ggplot(mk, aes(x = sen.slope)) + geom_histogram() + facet_wrap(~region, scales = 'free')
...
srMunged %>% filter(COMID == check$COMID[3], year > 1999, year < 2013) %>% ggplot(., aes(x = red, color = sat)) + geom_density()
