...
mk.full <- FieldMK %>%
  ...
  mutate(source = factor(source, levels = c('NLA.Sample', 'Random.Sample', 'Large.Lakes.Sample')))
...
trendDif <- mk.full %>%
  ...
  gather(NLA.minus.Random, NLA.minus.Large.Lakes, key = 'Group', value = 'Slope.Difference') %>%
  mutate(sig = NA, p.value = NA, source = 'Difference') %>%
  left_join(region %>% st_simplify(dTolerance = 500))


p1 <- ggplot(region %>% st_simplify(dTolerance = 500)) + geom_sf(aes(fill = region)) + 
  geom_sf(data = lakes, aes(color = 'Lakes'), size = .3, color = 'black', alpha = .4) + 
  theme(axis.text.x = element_blank(),
          axis.text.y = element_blank(),
          axis.ticks = element_blank(),
          rect = element_blank(),
...
p3 <- ggplot(trendDif %>% filter(Group == 'NLA.minus.Random')) +
...
p1 <- mk.full %>%
  ...
  ggplot() +
    ...
    scale_fill_viridis_c(option = 'plasma', start = .2) +
    #scale_fill_gradient2(low='#F56217', high='#0B486B') +
    labs(fill = 'Slope (cm/year)', x = '', y = '') +
    guides(color = guide_legend(label = F)) +
  facet_wrap(~source) +
...
mk <- summ.meds.full %>% filter(COMID %in% counts$COMID) %>% 
  group_by(COMID, region) %>%
  arrange(year) %>%
  nest() %>%
...
ggplot(mk, aes(x = sen.slope)) + geom_density(aes(fill = Size.Group), alpha = .4) + scale_fill_viridis_d(end = .8) + theme_bw() + ggtitle('Distribution of Lake Trends by Size Class') + geom_vline(data = mk.ave, aes(xintercept = mean, color = Size.Group)) + scale_color_viridis_d(end = .9) + coord_cartesian(xlim = c(-5,5))

t.test(mk$sen.slope[mk$Size.Group == '<10'], mk$sen.slope[mk$Size.Group == '10-100'])
t.test(mk$sen.slope[mk$Size.Group == '10-100'], mk$sen.slope[mk$Size.Group == '>100'])
t.test(mk$sen.slope[mk$Size.Group == '<10'], mk$sen.slope[mk$Size.Group == '>100'])
t.test(mk$sen.slope[mk$Size.Group == '>100'])
t.test(mk$sen.slope[mk$Size.Group == '10-100'])
t.test(mk$sen.slope[mk$Size.Group == '<10'])
