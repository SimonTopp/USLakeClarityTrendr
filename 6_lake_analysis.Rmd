...
summ.meds %>%
  filter(!is.na(value)) %>%
  group_by(COMID, year) %>%
  summarise(dataFrac = n()/1012) %>% ## or 13060 for random, 2270 for NLA
...
##Check for differences between NLA and random.

colors = viridis(9, direction = -1)
...
#OG was with EcoReg2000 not EceoReg200_cntr
...
#OG was with EcoReg2000 not EcoReg2000_cntr
...
ggsave('figures/TrendbySizeClassViolinV2.png', width = 2.5, height = 2.5, units = 'in')
...
##Try to make a heatmap of all the lakes
samp <- sample(unique(summ.meds.full$COMID), 1000)

summ.meds.full %>% 
  filter(COMID %in% samp) %>%
  left_join(mk %>% select(COMID, sen.slope, sig)) %>%
  filter(sig == T) %>%
  arrange(sen.slope) %>%
  ggplot(.) +
  geom_tile(aes(x = COMID, y = year, fill = secchi.med)) + 
  geom_point(aes(x = COMID, y = 2020, color = sen.slope)) +
...
## Quicky PCA experiment, doesn't really show very good clustering of trends
library("factoextra")
mkPca <- mkSig %>% ungroup() %>% select_if(is.numeric) %>% na.omit() %>%   select(inStreamCat, PctCarbResidCat, Precip8110Cat, Tmean8110Cat, RunoffCat, ClayCat, SandCat, OmCat, PermCat, RckdepCat, WtDepCat, BFICat, ElevCat, BFICat, KffactCat, CatAreaSqKm, PctAg2006Slp10Cat, ElevCat, NPDESDensCat, HydrlCondCat, PctImp2006Cat, pctUrban2006, pctForest2006, PctCrop2006Cat, pctWetland2006, WetIndexCat, areasqkm, meandused)
  
pcaTrendIndex <-  mkSig %>% ungroup() %>% select_if(is.numeric) %>% na.omit() %>% 
  mutate(trendClass = ifelse(sen.slope > 0, 'increasing', 'decrasing'))
#(trendClass = ifelse(p.value >0.05, 'none', ifelse(p.value <= 0.05 & sen.slope > 0, 'increasing', 'decrasing')))

pca <- prcomp(mkPca, center = T, scale = T)
fviz_pca_ind(pca, geom.ind = "point", pointshape = 21, 
             pointsize = 2, 
             fill.ind = pcaTrendIndex$trendClass,
             col.ind = "black", 
             palette = "jco", 
             #addEllipses = TRUE,
             label = "var",
...
##Slice and dice a la Tamlin
# Just look at end member lakes (highest and lowest trends 0-5 95-100 percentiles)
# Look at scatterplots, look for some more complexity.
## quickly just check out scatterplots
...
rf <- randomForest(mkPca, pcaTrendIndex$sen.slope, ntree = 200)
varImpPlot(rf)
tibble(predicted = rf$predicted, actual = rf$y) %>%
  ggplot(., aes(x = predicted, y = actual)) + geom_hex() + scale_fill_continuous(trans = 'log10') + geom_smooth() + geom_abline() + stat_poly_eq(aes(label =  paste(stat(adj.rr.label))),
               formula = y~x, parse = TRUE, 
               label.y = Inf, vjust = 1.3)

mae(rf$predicted, rf$y)

## Quick lm
lm <- lm(as.formula(paste0('sen.slope~',paste(features, collapse = '+'))), mkSig)
summary(lm)
...
tc %>% gather(PDO:NAO, key = 'tc', value = 'value') %>%
  ggplot(., aes(x = year, y = value, color = tc)) + geom_line() + 
  geom_vline(xintercept = c(1985,1987,1994,1995,2009,2011,1992))
...
##Quick test
boot.scaled <- bootstrapped.full %>%
  select(region, mean, year, Precip, Temp) %>% 
  group_by(region) %>%
  mutate_at(vars(mean, Precip, Temp), scale) %>%
  ungroup() %>%
  group_by(year) %>%
  summarise_at(vars(mean, Precip, Temp), mean, na.rm = T)

boot.scaled %>%
  gather(mean, Precip, Temp, key = 'Measure', value = "value") %>%
  ggplot(., aes(x = year, y = value, color = Measure)) + geom_line() +
  geom_vline(xintercept = c(1985,1987,1994,1995,2009,2011,1997,2016))


p.dif.scaled <- boot.scaled %>%
  gather(mean, Precip, Temp, key = 'Measure', value = "value") %>%
  group_by(Measure) %>% mutate(p.dif = ((value/lag(value))-1)*100)%>%
  ungroup() %>%
  select(-value) %>%
  spread(Measure, p.dif) %>%
  na.omit()
...
GGally::ggpairs(boot.scaled[,-1])
GGally::ggpairs(p.dif.scaled[,-1])
