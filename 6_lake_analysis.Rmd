...
lake.join <- read_feather('out/NLA2012LakesFull.feather') %>% filter(COMID %in% nla.2012$COMID) %>% mutate(sample = 'NLA') %>%
...
lake.join %>% filter(sample == 'NLA'|sample == 'Random', COMID %in% summ.meds.full$COMID) %>%
  ggplot() + geom_freqpoly(aes(x = areasqkm, color = sample)) + scale_x_log10() + facet_wrap(~region)
...
ggplot(mk %>% left_join(mk.ave), aes(x = Size.Group, y = sen.slope, group = Size.Group)) + 
  ...
  geom_violin(aes(fill = count), alpha = .6) + 
  geom_boxplot(width = .2, fill = 'grey60', alpha = .4) +
...
ggsave('figures/TrendbySizeClassViolin.png', width = 5, height = 2.5, units = 'in')
