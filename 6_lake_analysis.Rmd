...
## Framework for looking at trends relationships to various things
...
Featcors = function(i){
  ...
  lm = lm(f, mk)
  out = broom::tidy(lm) %>% .[2,]
  c = cor.test(c(mk[['sen.slope']]), mk[[i]], method = 'spearman') %>% broom::tidy()
  out$corr = c$estimate
  out$cor.p = c$p.value
  return(out)
}
