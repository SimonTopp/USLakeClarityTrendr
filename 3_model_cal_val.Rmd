...
p1 <- ggplot(output, aes(x = Actual, y = Predicted)) + 
  ...
  #scale_y_continuous(trans = 'log10', labels = scales::comma) +
  #coord_equal(ratio = 1) +
  labs(title = 'Observation Level Validation', x = 'Actual', y = 'Predicted')
...
p2 <- fieldmatch.coin %>%
  ...
  #scale_y_continuous(trans = 'log10', labels = scales::comma) +
  #coord_equal(ratio = 1) +
  labs(title = 'Aggregated Validation', x = 'Actual', y = 'Predicted')


p3 <- ggplot(fieldmatch.coin, aes(x = year, y = mean, color = Measure)) + 
  #geom_point(alpha = .1) +
  geom_line() + 
  scale_color_viridis_d(end = .6, labels = c('Actual', 'Predicted')) +
