...
p1 <- output.full %>% mutate(Size.Group = cut(areasqkm, breaks = c(-1,.1,1,10,100,100000), labels = c('<.1','.1-1','1-10', '10-100', '>100'))) %>% ggplot(aes(x = Actual, y = Predicted)) + 
  ...
  theme_bw() +
  labs(title = 'Hold-Out Validation by Lake Size (sq. km)', x = 'Actual', y = 'Predicted', tag = 'A)') + facet_wrap(~Size.Group)
...
p2 <- output.full %>% 
  ...
  stat_poly_eq(aes(label =  paste(stat(adj.rr.label))),
               formula = y~x, parse = TRUE, 
               label.y = Inf, vjust = 1.1, size = 3) +
  labs(title = 'Hold-Out Validation by Sensor', x = 'Actual', y = 'Predicted', tag = 'B)') +
  theme_bw() +
  facet_wrap(~sat, nrow = 1)

## By year
p3 <- output.full %>% group_by(year) %>% summarise(bias = bias(Actual, Predicted)) %>%
  ggplot(aes(x = year, y = bias)) + 
  geom_point() + geom_line() + 
  geom_smooth(method = 'lm') +
  theme_bw() + 
  labs(title = 'Hold-Out Bias by Year', tag = 'C)', x = 'Year', y = 'Bias (Actual-Predicted)') +
  ggpubr::stat_regline_equation(label.x.npc = 'middle', label.y.npc  = 'top')

layout <- rbind(c(1,1,1), c(1,1,1),c(1,1,1),c(2,2,2),c(2,2,2),c(3,3,NA), c(3,3,NA))
g <- grid.arrange(p1,p2,p3, layout_matrix = layout)

ggsave('figures/ValidationBreakDown.png', width = 6, height = 8, units = 'in', plot = g)
